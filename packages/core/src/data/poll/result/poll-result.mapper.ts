...
export const PollResultEntityMapper = {
  fromRaw: (rawData: Record<string, any>[] | undefined): PollResultEntity<PollType>[] => {
    if (!rawData) return [];
    return rawData.map((poll) => {
      const handler = getPollTypeHandler(poll.type);
      return {
        ...PollResultEntityMapper.fromRawBase(poll),
        results: handler.parseResults(poll.results),
      };
    });
  },
  fromRawBase: (rawData: Record<string, any>): PollResultEntityBase => {

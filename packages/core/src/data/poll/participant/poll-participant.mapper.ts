...
export const PollParticipantEntityMapper = {
  fromRaw: (rawData: Record<string, any>[] | undefined): PollParticipantEntity<PollType>[] => {
    if (!rawData) return [];
    return rawData.map((poll) => {
      const handler = getPollTypeHandler(poll.type);
      return {
        ...PollParticipantEntityMapper.fromRawBase(poll),
        vote: poll.vote ? handler.parseParticipant(poll.vote) : undefined,
      };
    });
  },
  fromRawBase: (rawData: Record<string, any>): PollParticipantEntityBase => {
